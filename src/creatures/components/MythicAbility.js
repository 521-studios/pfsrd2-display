...
const MythicAbility = (props) => {
  ...
  return (
    <div key={i}>
      <strong>{mythicAbility.name}</strong>
      ...
      {mythicAbility.mythic_activations && mythicAbility.mythic_activations.map((activation, j) => {
        ...
        return (
          <div key={j} className="MythicAbility__activation">
            ...
            {activation.cost ? <span>{' '}<strong>Cost</strong> {activation.cost}</span> : null}
            {activation.trigger ? <span>{' '}<strong>Trigger</strong> {activation.trigger}</span> : null}
            {activation.text ? <span>{' '}<Markdown text={activation.text} /></span> : null}
            {activation.effect ? <span>{' '}<strong>Effect</strong> <Markdown text={activation.effect} /></span> : null}
          </div>
        )
      })}
    </div>
